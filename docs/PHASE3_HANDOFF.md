...
- **Phase-4 kickoff ruling (operator, 2026-07-10) — REVISES "Phase 4 (multi-factor) EXCLUDED" above.** The six multi-factor blueprint combinations (P4.1 VWAP+Breakout+Volume · P4.2 ORB+VWAP · P4.3 Bollinger+RSI · P4.4 Adaptive-MA+ADX · P4.5 Pivot+MACD · P4.6 Donchian+ATR-stop) are **drafted + committed in ONE blind block** (with the §6 + no-lookahead landscape for all six), then **run incrementally from P4.1 in batches of 2–3 or until a STOP**, held for individual sign-off. **§6 is INFORMATIONAL ONLY in Phase 4 — no battery is skipped** (every combination runs the full panel battery; §6 is color, never a gate — this moots the single-symbol→panel mismatch and changes no threshold). Discipline unchanged: blueprint combinations only (no Phase-3-pattern substitutions), blind/a-priori (nothing from the gradient / sparse-big-move / `POST_PROJECT_DIRECTIONS.md`), **minimal AND-confluence / fewest knobs** (bull_flag PBO caution), no-lookahead precondition per composite, flag any near-mutually-exclusive confluence. Phase 4.5 + Phase-5 remain out of scope.
