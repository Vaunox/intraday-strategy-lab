...
Deterministic and point-in-time: the prior-N-bar high/low is the INTRADAY-RESET
Donchian channel (``intraday_donchian``) — the prior N bars WITHIN the current IST
trading day, excluding the current bar and never crossing the overnight gap — so an
early-session bar breaks TODAY's range, not a level carried from yesterday. Relative
volume uses only prior volumes, so bar ``i`` uses only bars ``0..i``; a signal at bar
``t``'s close fills at bar ``t+1``'s open (Inviolable Rule 2). The range is a
PRIOR-N-BAR range (the ``breakout_lookback`` parameter), which keeps P3.2 distinct
from Opening Range Breakout (P3.11, the day's running high/low): using an expanding
day-range here would collapse the two studies into one correlated bet.
...
from lab.data.features.indicators import intraday_donchian, relative_volume
...
class BreakoutSpec:
    ...
    def generate_signals(self, candles: Sequence[Candle]) -> list[StrategySignal]:
        ...
        for i, candle in enumerate(candles):
            ...
            # Prior N-bar high/low WITHIN today, as of bar i (excludes i); NaN at the day's
            # first bar, so an early-session bar cannot break a level carried from yesterday.
