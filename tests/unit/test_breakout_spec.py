...
def test_intraday_reset_ignores_prior_day_range_early_in_session() -> None:
    """The intraday-reset range: an early day-2 bar breaks TODAY's range, not day 1's.

    Under a GLOBAL Donchian the first day-2 bar (close 101.0) would break day 1's 100.5
    high across the overnight gap; the reset gives day 2 no intraday range yet (NaN), so
    that cross-gap false break is silenced -- the signal only fires once day 2 has its own
    range to break.
    """

    def _day2_bar(i: int, high: float, low: float, close: float, volume: int) -> Candle:
        ts = datetime(2024, 7, 16, 9, 15, tzinfo=IST) + timedelta(minutes=5 * i)
        return Candle("RELIANCE", BarInterval.MIN_5, ts, close, high, low, close, volume)

    candles = [
        *_range3(),  # day 1: a [99.5, 100.5] range
        _day2_bar(0, 101.5, 100.0, 101.0, 3000),  # day-2 open: no prior intraday range -> silent
        _day2_bar(1, 102.0, 100.5, 101.8, 3000),  # breaks day 2's own bar-0 high (101.5) -> LONG
    ]
    signals = SPEC.generate_signals(candles)
    assert [s.side for s in signals] == [Side.LONG]
    assert (
        signals[0].asof == candles[4].timestamp
    )  # the day-2 intraday break, not the cross-gap one
